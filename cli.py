...
from cli_commands.show_range import show_range
from cli_commands.setup_database import setup_database
...
class RichGroup(click.Group):
    def format_help(self, ctx, formatter):
        ...
        t.add_row(['setup-database', 'action', 'None', 'Creates an empty database with the appropriate schema'])
        t.add_row(['', '', '', ''])
        ...
        t.add_row(['show-range', 'plot', '(required) player name (str)\n(required) position: blinds | early | middle | late', 'Shows the range of hands observed \nfor a particular player'])
        t.add_row(['', '', '', ''])
        t.add_row(['show-last-n-tournaments', 'table', '(required) n', 'Shows the last n tournaments'])
...
main.add_command(show_range)
main.add_command(setup_database)
