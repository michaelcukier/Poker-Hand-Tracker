...
def sql_query(database_file_path: str) -> PrettyTable:
    ...
    # for d in data3:
    #     print(d)
    # t = PrettyTable(['buyin', 'avg'])
    #
    # for buyin, avg in data3:
    #     t.add_row([buyin, avg])

    t = PrettyTable(['Buyin', 'ITM %', 'Net', 'Invested', 'Profit', 'Count', 'Avg. profit/game'])

    for buyin, itm, totalPrizes, totalPrices, count, profit, avgProfitPerGame in data3:
        t.add_row([buyin, itm, totalPrizes, totalPrices, count, profit, avgProfitPerGame])

    return t
    # example:

    # Buyin | ITM % | Total profit | Total invested | ROI |  Avg. profit / game | Count | Avg. position | Avg. duration
